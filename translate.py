...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        model = create_or_load_model(sess, model, True)
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.decode_input:
            ...
            while i < len(inputs):
                ...
                e_in_len = len(next_batch_encoder_inputs[0])
                d_in_len = len(next_batch_decoder_inputs[0])
                t_in_len = len(next_batch_target_weights[0])

                reshaped_next_batch_encoder_inputs = np.array([[a[j] for a in next_batch_encoder_inputs] for j in range(e_in_len)], dtype=np.int32)
                reshaped_next_batch_decoder_inputs = np.array([[a[j] for a in next_batch_decoder_inputs] for j in range(d_in_len)], dtype=np.int32)
                reshaped_next_batch_target_weights = np.array([[a[j] for a in next_batch_target_weights] for j in range(t_in_len)], dtype=np.int32)
            ...
            with open(FLAGS.decode_output, "w") as output_file:
                for i in range(len(batch_encoder_inputs)):
                    ...
                    try:
                        ...
                        # If there is an EOS symbol in outputs, cut them at that point.
                        outputs = [output[:list(output).index(data_utils.EOS_ID)] if data_utils.EOS_ID in output else output for output in outputs]
