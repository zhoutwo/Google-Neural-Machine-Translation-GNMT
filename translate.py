...
def train():
    ...
    while True:
        ...
        writer.add_summary(tf.summary.scalar(name="generator_normal_loss", tensor=tf.convert_to_tensor(step_loss)), global_step=global_step).eval()
        with g_eval.as_default():
        ...
        writer.add_summary(tf.summary.scalar(name="discriminator_truth_loss", tensor=tf.convert_to_tensor(dis_loss)),
        ...
        writer.add_summary(tf.summary.scalar(name="discriminator_composed_loss", tensor=tf.convert_to_tensor(composed_dis_loss)),
        ...
        with g_train.as_default():
            ...
            global_step = train_model.global_step.eval(session=train_sess)
        writer.add_summary(tf.summary.scalar(name="generator_composed_loss", tensor=tf.convert_to_tensor(new_step_loss)),
                           global_step=global_step).eval()
        ...
        if current_step % FLAGS.steps_per_checkpoint == 0:
            ...
            writer.add_summary(tf.summary.scalar(name="learn_rate", tensor=train_model.learning_rate),
                               global_step=global_step).eval()
            writer.add_summary(tf.summary.scalar(name="train_step_time", tensor=tf.convert_to_tensor(step_time)),
                               global_step=global_step).eval()
            writer.add_summary(tf.summary.scalar(name="train_perplex", tensor=tf.convert_to_tensor(perplexity)),
                               global_step=global_step).eval()
            ...
            for bucket_id in range(len(_buckets)):
                ...
                writer.add_summary(tf.summary.scalar(name="eval_perplex_bucket_"+str(bucket_id), tensor=tf.convert_to_tensor(eval_ppx)),
